(function () {
  ...
  function initAccountPerformanceChart() {
    const d = window.accountPerformanceChart || {};
    const el = document.getElementById('accountPerformanceChart');
    ...
    new Chart(el, {
      ...
      data: {
        labels,
        datasets: [
          lineDataset('Amount Taken', d.taken, '#1a73e8', { fill: true, bold: true }),
          lineDataset('Cash In Hand', d.cash, '#34a853', { fill: true, bold: true }),
          lineDataset('Previous Balance', d.previous, '#9aa0a6'),
          lineDataset('Balance After', d.after, '#f9ab00'),
        ],
      },
      options: {
        responsive: true,
