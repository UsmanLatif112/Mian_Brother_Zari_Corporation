(function () {
  ...
  function initJournalPerformanceChart() {
    const d = window.journalPerformanceChart || {};
    const el = document.getElementById('journalPerformanceChart');
    if (!el) return;
    ...
    new Chart(el, {
      ...
      data: {
        labels,
        datasets: [
          lineDataset('Total In', d.in, '#34a853', { fill: true, bold: true }),
          lineDataset('Total Out', d.out, '#ea4335'),
          lineDataset('Net', d.net, '#1a73e8', { fill: true, bold: true }),
        ],
      },
      options: {
        responsive: true,
