...
from app.services.sale_service import create_sale, update_sale, void_sale
...
def index():
    ...
    # One-shot: open modal once, then clear so refresh does not reopen it
...
def create():
    ...
    for line in items_raw:
        ...
        if not product:
            return jsonify({"ok": False, "error": "Invalid product selected."}), 400
        qty = Decimal(str(line.get("quantity") or 0))
        if qty <= 0:
            return jsonify({"ok": False, "error": f"Invalid quantity for {product.name}."}), 400
        items.append(
            {
                "product": product,
                "quantity": qty,
    ...
    if payment_status == "unpaid":
        ...
        # Paid: default to full total; allow overpay for customer advance
        amount_paid = Decimal(
            str(
                data.get("amount_paid")
                if data.get("amount_paid") not in (None, "")
                else grand_estimate
            )
        )
        payment_method = "cash"
    ...
    if needs_customer and not customer_id:
        return jsonify(
            {
                "ok": False,
                "error": "Select a customer for credit, partial, or overpayment sales.",
            }
        ), 400
    ...
    try:
        ...
        log_audit("create", "sale", sale.id, json.dumps(audit_payload, ensure_ascii=False))
        db.session.commit()
        return jsonify(
            {
                "ok": True,
...
@permission_required("sales.create")
def pos_submit():
    # Backward-compatible alias to create endpoint
    return create()
